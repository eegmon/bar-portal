...
export default async function AssemblyVoteServerPage({
        ...
        <div className="p-8 bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl text-center space-y-5">
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs text-slate-400 leading-relaxed">
              총회 전자투표는 도스변호사협회에 등록된 <strong>정회원 변호사 계정</strong>으로 로그인하신 후 의결권을 행사하실 수 있습니다.
            </p>
          </div>
          <div className="pt-2">
            <Link
  ...
  try {
    // 안건 목록 조회
    const assemblyRes = await db.execute("SELECT * FROM assemblies ORDER BY round_number DESC");
    assemblies = assemblyRes.rows;
    const selectedAssemblyId = assemblyId || "";
    const agRes = await db.execute({ sql: "SELECT * FROM agendas WHERE assembly_id = ? ORDER BY agenda_order ASC, created_at ASC", args: [selectedAssemblyId] });
    agendas = agRes.rows;
    const selectedAgenda = agendas.find((agenda) => agenda.id === (agendaId || agendas[0]?.id));
    ...
    votingPower = Number(ownRightRes.rows[0]?.voting_power || 1) + proxyList.length;
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10 w-full space-y-8">
      {/* 헤더 */}
      <div>
        <Link href="/assembly" className="text-slate-400 hover:text-white text-xs flex items-center gap-1 mb-1">
      ...
      <form method="get" className="p-4 bg-slate-900 border border-slate-800 rounded-xl flex flex-wrap items-center gap-3">
        <label htmlFor="assemblyId" className="text-xs font-bold text-white">투표할 총회 선택</label>
        <select id="assemblyId" name="assemblyId" defaultValue={assemblyId || ""} className="flex-1 min-w-60 bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-sm text-white">
          ...
          {assemblies.map((item) => <option key={item.id} value={item.id}>{item.title} · {item.held_at}</option>)}
        </select>
      ...
      {assemblyId && agendas.length > 0 ? <VoteClient
        user={user}
      ...
      /> : <div className="p-10 bg-slate-900 border border-slate-800 rounded-xl text-center text-sm text-slate-500">총회를 선택하면 해당 총회의 표결 안건이 표시됩니다.</div>}
    </div>
  );
}
