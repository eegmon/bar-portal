...
export async function POST(req: Request) {
  try {
    ...
    if (!securityCode) {
      return NextResponse.json({ error: "수험번호(보안코드)를 입력해 주세요." }, { status: 400 });
    }
    ...
    if (res.rows.length === 0) {
      return NextResponse.json({
        eligible: false,
        error: "해당 수험번호의 1차 CBT 응시 기록이 없습니다. 1차 시험을 먼저 응시해 주세요.",
      }, { status: 403 });
    }
    ...
    return NextResponse.json({ error: err.message || "자격 검증 실패" }, { status: 500 });
  }
}
