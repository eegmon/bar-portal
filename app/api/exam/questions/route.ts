...
export async function GET() {
  try {
    const res = await db.execute({
      sql: "SELECT id, title, round_number, phase1_questions, errata_notices, status FROM exams ORDER BY round_number DESC LIMIT 1",
    });

    if (res.rows.length === 0) {
      return NextResponse.json({ error: "등록된 시험이 없습니다." }, { status: 404 });
    }
