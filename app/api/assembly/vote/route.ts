...
export async function GET(req: Request) {
  try {
    ...
    const presentRes = await db.execute({ sql: "SELECT COALESCE(SUM(CASE WHEN attended = 1 OR is_proxy = 1 THEN 1 ELSE 0 END), 0) as present_rights FROM assembly_attendances WHERE assembly_id = ? AND approval_status = 'APPROVED'", args: [agenda.assembly_id] });
    const castRes = await db.execute({ sql: "SELECT COALESCE(SUM(votes_count), 0) as casted FROM ballot_box WHERE agenda_id = ?", args: [agendaId] });
    const voterRes = await db.execute({ sql: "SELECT COUNT(*) as voters FROM voter_logs WHERE agenda_id = ?", args: [agendaId] });
    const tallyRes = await db.execute({ sql: "SELECT choice, SUM(votes_count) as total FROM ballot_box WHERE agenda_id = ? GROUP BY choice", args: [agendaId] });
    ...
    const stats = {
      ...
      quorumMet: presentRights >= Number(agenda.quorum_needed || Math.ceil(totalRights / 3)),
      tally: tallyRes.rows.map((row) => ({ choice: row.choice, total: Number(row.total || 0) })),
    };
    if (!canManageAssembly(user) && !["CLOSED", "RESULT_CONFIRMED"].includes(String(agenda.status))) stats.tally = [];
    return NextResponse.json({ success: true, stats });
  } catch (err: unknown) {
    console.error("투표 현황 조회 에러:", err);
    return NextResponse.json({ error: "투표 현황 조회에 실패했습니다." }, { status: 500 });
  }
}
...
export async function POST(req: Request) {
  try {
    ...
    const user = await getSessionUser();
    if (!user) {
      return NextResponse.json({ error: "총회 전자투표는 로그인한 회원 변호사만 참여할 수 있습니다." }, { status: 401 });
    }

    if (user.role !== "LAWYER" && user.role !== "ADMIN") {
      return NextResponse.json({ error: "변호사 자격이 있는 정회원만 투표권이 있습니다." }, { status: 403 });
    }

    if (user.status !== "ACTIVE") {
      return NextResponse.json({ error: `현재 자격 상태(${user.status})로는 의결권을 행사할 수 없습니다.` }, { status: 403 });
    }
    ...
    if (!agendaId || !allocations || Object.keys(allocations).length === 0) {
      return NextResponse.json({ error: "투표 데이터가 누락되었습니다." }, { status: 400 });
    }
    ...
    if (agenda.voting_deadline && new Date(String(agenda.voting_deadline)).getTime() <= Date.now()) {
      await db.execute({
        sql: "UPDATE agendas SET status = 'CLOSED', voting_closed_at = datetime('now') WHERE id = ? AND status = 'VOTING'",
        args: [agendaId],
      });
      return NextResponse.json({ error: "투표 마감 시간이 지났습니다." }, { status: 400 });
    }

    const allowedChoices = getChoices(agenda.choice_config);
    if (agenda.voting_method === "RANKED") {
      if (!Array.isArray(ranking) || ranking.length !== allowedChoices.length || new Set(ranking).size !== ranking.length || ranking.some((choice: unknown) => !allowedChoices.includes(String(choice)))) {
        return NextResponse.json({ error: "모든 선택지를 한 번씩 순위에 배치해야 합니다." }, { status: 400 });
      }
    }
    for (const [choice, rawCount] of Object.entries(allocations as Record<string, unknown>)) {
      const count = Number(rawCount);
      if (!allowedChoices.includes(choice) || !Number.isInteger(count) || count < 0) {
        return NextResponse.json({ error: "허용되지 않은 선택지 또는 표 수입니다." }, { status: 400 });
      }
    }
    ...
    if (checkLog.rows.length > 0) {
      return NextResponse.json({ error: "이미 본 안건에 표결을 완료하셨습니다. (중복 투표 불가)" }, { status: 400 });
    }
    ...
    // 행사하려는 표 수 합계 검증
    const totalCasted = agenda.voting_method === "RANKED"
      ? calculatedVotingPower
      : Object.values(allocations as Record<string, unknown>).reduce<number>((sum, rawCount) => sum + Number(rawCount), 0);
    ...
    if (agenda.voting_method === "RANKED") {
      voteStatements.push({
        ...
        args: [`ballot-${Date.now()}-${Math.random().toString(36).substring(2, 7)}`, agendaId, `__RANKING__:${JSON.stringify(ranking)}`, calculatedVotingPower],
      });
    }
    for (const [choice, count] of Object.entries(allocations as Record<string, unknown>)) {
